...
생성자 | ```swift  init(progressViewStyle: UIProgressView.Style)  init(frame: CGRect)  init?(coder: NSCoder)```
주요 함수 | `func setProgress(Float, animated: Bool)`
예제 | ```swift  let myProgressView: UIProgressView = UIProgressView(frame: CGRect(x:0, y:0, width:100, height:5))  myProgressView.setProgress(0.3, animated: true)```
